...
soyk <- read_xlsx("../01_soybeans.xlsx")


# Create the myplot function
myplot <- function(df, x, y) {
  ggplot(df, aes(x = as.factor(.data[[x]]), y = .data[[y]])) +
    geom_boxplot(fill = "turquoise", color = "black") +
    labs(x = x, y = y, title = paste("Boxplot for", y, "by", x)) +
    theme_minimal()
}

# Apply the function to create a boxplot for 'Midville K pct' as a function of 'K rate (lbac)'
myplot(soyk, 'K rate (lbac)', 'Macon K pct')
...
plot_location <- function(location) {
  ggplot(data, aes(x = as.factor(`K rate (lbac)`), y = .data[[location]], fill = as.factor(TRT))) +
    geom_boxplot() +
    labs(title = paste("Boxplot for", location, "by K rate (lbac)"),
         x = "K rate (lbac)",
         y = "Potassium Percentage",
         fill = "TRT") +
    theme_minimal()
}


data_locations <- c("Tifton K pct", "Midville K pct", "Watkinsville K pct", "Macon K pct")


plots <- map(data_locations, plot_location)
